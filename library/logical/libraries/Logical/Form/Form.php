...
class Form
{
	...
	/**
	 *  Method to check if the form has an xml element.
	 */
	protected function hasXml()
	{
		// Make sure there is a valid JForm XML document.
		if (!($this->xml instanceof \SimpleXMLElement))
		{
			throw new \UnexpectedValueException(sprintf('%s `xml` is not an instance of SimpleXMLElement', get_class($this)));
		}
	}
	...
	protected function bindLevel($group, $data)
	{
		...
		{
			$level = $this->getGroupName($name, $group);
	...
	public function filter($data, $group = null)
	{
		...
		if (!$fields)
		{
			throw new \ErrorException('Failed to load form fields');
		}
	 ...
	 * @param   mixed  $default The optional value to use as the default for the field.
	 * @param   string $group   The optional dot-separated form group path on which to find the field.
	 *
	 * @return  \JFormField|boolean  The JFormField object for the field or boolean false on error.
	 *
	 */
	public function getField($name, $default = null, $group = null)
	{
		// Make sure there is a valid JForm XML document.
		if (!($this->xml instanceof \SimpleXMLElement))
		{
		...
		return $this->loadField($element, $default, $group);
	}
	...
	{
		$fields = array();
		$elements = $this->getFieldsetElements($set);
		...
		foreach ($elements as $element)
		{
			...
			if ($field = $this->loadField($element, null, $group))
			{
	...
	/**
	 * Method to get an array of XML elements
	 *
	 * @param   string $set The optional name of the fieldset.
	 *
	 * @return bool|\SimpleXMLElement[]
	 */
	protected function getFieldsetElements($set = null)
	{
		// Get all of the field elements in the fieldset.
		if ($set)
		{
			return $this->findFieldsByFieldset($set);
		}

		return $this->findFieldsByGroup();
	}
	...
	public function getFieldsets($group = null)
	{
		...
		$sets = $this->getSetsByGroup($group);
		...
		foreach ($sets as $set)
		{
			...
			$fieldset = $this->getFieldsetAsObject($set, $fieldsets);
	...
	/**
	 * Method to get an array of xml elements by group
	 *
	 * @param null $group
	 *
	 * @return array|\SimpleXMLElement[]
	 */
	protected function getSetsByGroup($group = null)
	{
		if (empty($group))
		{
			// Get an array of <fieldset /> elements and fieldset attributes.
			return $this->xml->xpath('//fieldset[@name and not(ancestor::field/form/*)] | //field[@fieldset and not(ancestor::field/form/*)]/@fieldset');
		}
	...
	/**
	 * Method to convert a xml fieldset definition into an object
	 *
	 * @param   \SimpleXMLElement $set       either an fieldset element or an attribute element
	 * @param   array             $fieldsets that have already been included.
	 *
	 * @return object|null
	 */
	protected function getFieldsetAsObject($set, $fieldsets = array())
	{
		...
		// Must be dealing with a fieldset name value.
		...
		if (!empty($tmp))
		{
			return $this->getFieldsetAsObject($tmp, $fieldsets);
		}

		//If no element was found return a simple object
		return (object) array('name' => (string) $set, 'label' => '', 'description' => '');
	}
	...
	public function getInput($name, $group = null, $value = null)
	{
		...
		if ($field = $this->getField($name, $value, $group))
		{
			return $field->input;
		}
	 ...
	 */
	public function getLabel($name, $default = null, $group = null)
	{
		// Attempt to get the form field.
		if ($field = $this->getField($name, $default, $group))
		{
			return $field->label;
		}
	...
	{
		$field = $this->getField($name, $default, $group);
	 ...
	 *
	 * @param   string $xmlData   The name of an XML string or object.
	 ...
	 */
	public function load($xmlData, $replace = true, $xpath = false)
	{
		// If the data to load isn't already an XML element or string return false.
		if ((!($xmlData instanceof \SimpleXMLElement)) && (!is_string($xmlData)))
		{
			return false;
		}
		...
		if (is_string($xmlData))
		{
			...
			if (!$xmlData)
			{
				throw new \ErrorException('Unable to load xml data');
			}
		}
		...
		$elements = $this->getElementsToLoad($xmlData, $xpath);
	 ...
	 * @return array|\SimpleXMLElement
	 */
	protected function getElementsToLoad($xmlData, $xpath = null)
	{
		if ($xpath)
		{
			return $xmlData->xpath($xpath);
		}

		if ($xmlData->getName() == 'form')
		{
			return $xmlData->children();
		}
	 ...
	 * to false.
	 *
	 * @param   string $filename The filesystem path of an XML file.
	 ...
	 * @throws \ErrorException
	 */
	public function loadFile($filename, $reset = true, $xpath = false)
	{
		// Check to see if the path is an absolute path.
		if (is_file($filename))
		{
			return $this->load(simplexml_load_file($filename), $reset, $xpath);
		}

		// Not an absolute path so let's attempt to find one using JPath.
		$filename = \JPath::find(self::addFormPath(), strtolower($filename) . '.xml');

		// If unable to find the file return false.
		if (!$filename)
		{
			throw new \ErrorException('Unable to find file');
		}
	 ...
	 *
	 */
	public function setValue($name, $value = null, $group = null)
	{
	...
	public function validate($data, $group = null)
	{
		...
		if (!$fields)
		{
			throw new \ErrorException('Failed to load form fields loaded');
		}
	...
	{
		//@TODO refactor this once I've rebuild the rules, filters system
		...
		switch (strtoupper($filter))
		{
			...
			case 'RULES':
				return $this->rulesFilter($element, $value);
				break;
			...
			case 'INT_ARRAY':
				return $this->intArrayFilter($element, $value);
				break;
			...
			case 'SERVER_UTC':
				...
				return $this->utcFilter($element, $value, $offset);
			...
			case 'URL':
				return $this->urlFilter($element, $value);

				break;

			case 'TEL':
				return $this->telFilter($element, $value);
				break;
			default:

				return $this->defaultFilter($element, $value, $filter);
	...
	/**
	 * Method to filter value as RULE
	 *
	 * @param   string $element The XML element object representation of the form field.
	 * @param   mixed  $value   The value to filter for the field.
	 *
	 * @return array
	 */
	protected function rulesFilter($element, $value)
	{
		$rules = array();
	...
	/**
	 * Method to filter value as int array
	 *
	 * @param   string $element The XML element object representation of the form field.
	 * @param   mixed  $value   The value to filter for the field.
	 *
	 * @return array
	 */
	protected function intArrayFilter($element, $value)
	{
	...
	/**
	 * Method to filter value as utc date
	 *
	 * @param   string $element The XML element object representation of the form field.
	 * @param   mixed  $value   The value to filter for the field.
	 * @param   string $offset  timezone offset to use for the conversion.
	 *
	 * @return string
	 *
	 * @throws \Exception
	 */
	protected function utcFilter($element, $value, $offset)
	{
		if ((int) $value <= 0)
		{
			return '';
		...
		try
		{
			$date = \JFactory::getDate($value, $offset)->toSql();
	...
	/**
	 * Method to filter as uniform resource locator
	 *
	 * @param   string $element The XML element object representation of the form field.
	 * @param   mixed  $value   The value to filter for the field.
	 *
	 * @return bool|string
	 */
	protected function urlFilter($element, $value)
	{
		if (empty($value))
		{
			return false;
		...
		if ($protocol)
		{
			return \JStringPunycode::urlToPunycode($value);
		}
		...
		if (!$element['relative'])
		{
			...
			$value = (substr($value, 0, 9) == 'index.php') ? \JUri::root() . $value : $protocol . '://' . $value;
	...
	/**
	 * Method to filter as telephone number
	 *
	 * @param   string $element The XML element object representation of the form field.
	 * @param   mixed  $value   The value to filter for the field.
	 *
	 * @return string
	 */
	protected function telFilter($element, $value)
	{
		$value = trim($value);

		$telFormat = '/[0-9]{1,3}\.[0-9]{4,14}$/';
		$nanpFormat = '/^(?:\+?1[-. ]?)?\(?([2-9][0-8][0-9])\)?[-. ]?([2-9][0-9]{2})[-. ]?([0-9]{4})$/';
		$ituTFormat ='/^\+(?:[0-9] ?){6,14}[0-9]$/';
		$eppFormat ='/^\+[0-9]{1,3}\.[0-9]{4,14}(?:x.+)?$/';

		if (preg_match($telFormat, $value) == 1)
		{
			return $value;
		}

		if (preg_match($nanpFormat, $value) == 1)
		{
		...
		if (preg_match($ituTFormat, $value) == 1)
		{
		...
		if (preg_match($eppFormat, $value) == 1)
		{
			if (strstr($value, 'x'))
			{
				$xpos = strpos($value, 'x');
		...
		if ($value != null && strlen($value) <= 15)
		{
			...
			$cclen = $length - 12;
			return substr($value, 0, $cclen) . '.' . substr($value, $cclen);
		}
	 ...
	 * @param   string  $element The XML element object representation of the form field.
	 * @param   mixed   $value   The value to filter for the field.
	 * @param   string  $filter  Filter name from the element
	 *
	 * @return string
	 */
	protected function defaultFilter($element, $value, $filter)
	{
	 ...
	 */
	protected function loadField($element, $value = null, $group = null)
	{
		// Make sure there is a valid SimpleXMLElement.
		if (!($element instanceof \SimpleXMLElement))
		{
